...
public:
  ...
  bruteforce() = default;
  bruteforce(unsigned number_hint) {
  ...
  std::vector<shape_ptr> many_to_many() {
    ...
    std::transform(in_collision.begin(), in_collision.end(), std::back_inserter(result), [&](const auto &elem) { return std::addressof(m_stored_shapes[elem]); });
    return result;
  }
};
