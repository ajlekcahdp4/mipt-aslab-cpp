...
def generate_random_point(p_halfwidths: np.array, p_round: int, p_dim: int) -> np.array:
    return np.array(np.around(np.random.default_rng().uniform(-1 * p_halfwidths, 1 * p_halfwidths, p_dim), p_round))
        ...
        for c in range(group['number']):
            ...
            for i in range(length):
                points.append(generate_random_point(
                    halflengths, group['round'], 4))

            queries = []
            num_queries = group['queries']
            for i in range(num_queries):
                queries.append(generate_random_point(
                    halflengths, group['round'], 4))

            ans = generate_ans(points, queries)

            save_test_ans(p_config['output_path'], generate_test_string(points, queries, int(
                group['round'])), generate_ans_string(ans), c, group['test_fmt_string'], group['ans_fmt_string'])
