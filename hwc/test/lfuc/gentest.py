...
def save_test_ans(output_path: str, p_test: list, p_size: int, p_idx: int, p_test_fmt: str = 'test{}.dat', p_ans_fmt: str = 'ans{}.dat') -> None:
...
def generate_triangle(p_config: dict, p_old_idx: int) -> int:
    ...
    for i in range(triangle_dict['number']):
        sample = list(
            map(int, list(np.random.default_rng().triangular(left=left, right=right, mode=mode, size=length))))
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate belady end-to-end tests.')
